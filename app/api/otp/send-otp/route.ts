...
export async function POST(request: Request) {
  try {
    const { email, pid} = await request.json()

    if (!email || !pid) {
      return Response.json({ error: "Email and Player ID are required" }, { status: 400 })
    }
    ...
    await sql`DELETE FROM public.otp WHERE pid = ${pid} AND email = ${email} AND verified = FALSE`

    // Insert new OTP
    const result = await sql`
      INSERT INTO public.otp (pid, email, otp_code, expires_at, create_at, verified)
      VALUES (${pid}, ${email}, ${otpCode}, ${expiresAt},${created_at}, FALSE)
      RETURNING pid, otp_code, expires_at
    `
